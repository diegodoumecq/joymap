...
export default async function setupRotatingLogo(canvas: HTMLCanvasElement) {
  const gfxOptions = {
    deviceTypes: ['webgpu'],
  ...
  const assets = {
    frog: new pc.Asset('logo', 'texture', { url: getImgData() }),
  };
  ...
  assetListLoader.load(() => {
    ...
    // FIXME: apparently assets are not textures, but it works
    // @ts-ignore
