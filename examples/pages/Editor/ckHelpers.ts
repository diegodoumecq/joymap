...
export function executeCommand(editor: ClassicEditor, commandName: string, value?: CommandValue) {
  if (commandName === 'move') {
    setCursor(editor.model, value);
  } else if (commandName === 'select') {
    setSelection(editor.model, value);
  } else {
    if (value) {
      editor.execute(commandName, typeof value === 'function' ? value() : value);
    } else {
